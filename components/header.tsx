...
export default function Header() {
  ...
  const [esOrganizador, setEsOrganizador] =
    useState(false);
  ...
  const [eventosVueltas, setEventosVueltas] =
    useState<any[]>([]);

  const [eventosNormales, setEventosNormales] =
    useState<any[]>([]);

  const [openLive, setOpenLive] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const unsubVueltas =
      onSnapshot(
        ...
        (error) => {
          console.error(
            "Error cargando eventos por vueltas:",
            error
          );
        }
      );
    ...
    const unsubNormales =
      onSnapshot(
        ...
        (error) => {
          console.error(
            "Error cargando eventos normales:",
            error
          );
        }
      );
